...
VCtrig VoltReg_trigger(mat Dl, cx_mat Z, int Nnum, boost::posix_time::ptime timer_overvolt, boost::posix_time::ptime timer_undervolt, int flag)
{
	VCtrig vc_trig;
	...
	int re, trigger;
	boost::posix_time::ptime timer_overv, timer_underv, current_time;
	boost::posix_time::time_duration td_overv, td_underv;
	...
	if (Vmax_orig > 1.05 + 0.0001)
	{
		...
		//timer_underv = 0;
		timer_overv = timer_overvolt;
		timer_underv = current_time;
		flag = 1;
	}
	else if (Vmin_orig < 0.95 - 0.0001)
	{
		...
		//timer_overv = 0;
		timer_underv = timer_undervolt;
		timer_overv = current_time;
		flag = 1;
	}
	else
	{
	...
	td_overv = current_time - timer_overv;
	td_underv = current_time - timer_underv;
	if ((td_overv.minutes() == 15 && td_overv.seconds() > 0) || (td_underv.minutes() == 15 && td_underv.seconds() > 0) || (td_overv.minutes() > 15 || td_underv.minutes() > 15))
