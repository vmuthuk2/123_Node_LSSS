...
void VVCAgent::vvc_slave()
{
  ...
  cout << "Size of Grad_slave1 is: " << Grad_slave1.n_rows << "*" << Grad_slave1.n_cols << endl;
  //bool status = Grad_slave1.load("Grad_slave1.mat");
  //if (status == true)
  if (m_Synchronized)
  {
	  grad_value = Grad_slave1;
	  std::cout << "Gradient Values: \n" << grad_value << std::endl;
	  std::cout << "loaded okay" << std::endl;
	  sstA_cmd = mean(mean(join_cols(Grad_slave1.row(6),Grad_slave1.row(9))));
	  sstB_cmd = mean(mean(join_cols(Grad_slave1.row(7),Grad_slave1.row(10))));
	  sstC_cmd = mean(mean(join_cols(Grad_slave1.row(8),Grad_slave1.row(11))));
	  //m_Synchronized = 0;
  }
  else
  {
  ...
  std::cout << "SSTs under Slave #1: SST1 SST2 \n" << endl;
  ...
  catch(std::exception & e)
  {
	...
	if (m_Synchronized)
	{
		BOOST_FOREACH(CPeerNode peer, m_peers | boost::adaptors::map_values)
		{
		    ...
		    mat S2;
		    S2 << sstA_cmd << sstB_cmd << sstC_cmd << endr;
		    S2 = S2.t();
		    //cout << "S2 size is: " << S2.n_rows << "*" << S2.n_cols << endl;
