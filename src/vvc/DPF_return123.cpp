...
VPQ DPF_return123(mat Dl, cx_mat Z)
{
	VPQ dpf_return123;
	double bkva = 10000;
	double bkv = 4.16;
	double vo_orig = 4.16*1;
	double vo;
	double eps = 0.000001;
	int mxitr = 100;
	...
	/*Check if first node is LTC or VR*/
	int lcd1 = Dl(0, 3);
	if (lcd1 > 13)
	{
		vo = vo_orig*Dl(0, 13);
	...
	for (int i = 0; i < mxitr; i++)
	{
		...
		//		if (ln(l) != 0 && sbus(l) == 0)
		//		{
		//			Sb_new(ln(l)-1, a) = (bkva / 3)*V0(0, a)*conj(Ib(ln(l)-1, a));
		//		}
		//		else if (ln(l) == 0)
		//		{
		...
		//		else
		//		{
		//			Sb_new(ln(l),a) = (bkva / 3)*V(sbus(l)-1,a)*conj(Ib(ln(l)-1, a));
		//		}
		//	}
		//}
	...
	cx_mat Sbus_new = cx_mat(zeros(cnt_nodes - 1, 3), zeros(cnt_nodes - 1, 3));
	for (int l = 0; l < Nl; l++)
	{
		if (ln(l) != 0 && sbus(l) == 0)
		{
			Sbus_new.row(ln(l) - 1) = (bkva / 3)*V0 % conj(Iinj.row(ln(l) - 1));
		}
		else if (ln(l) == 0)
		{
		...
		{
			Sbus_new.row(ln(l) - 1) = (bkva / 3)*V_nodes.row(sbus(l)) % conj(Iinj.row(ln(l) - 1));
		}
	}
	...
	//mat PQbus = join_rows(join_rows(join_rows(real(Sbus.col(0)), imag(Sbus.col(0))), join_rows(real(Sbus.col(1)), imag(Sbus.col(1)))), join_rows(real(Sbus.col(2)), imag(Sbus.col(2))));
	mat PQbus = join_rows(join_rows(join_rows(real(Sbus_new.col(0)), imag(Sbus_new.col(0))), join_rows(real(Sbus_new.col(1)), imag(Sbus_new.col(1)))), join_rows(real(Sbus_new.col(2)), imag(Sbus_new.col(2))));
	//cout << "PQbus = \n" << PQbus << endl;
	...
	mat Vabs0 = abs(V0);
	mat Vang0 = (180 / datum::pi)*atan(imag(V0) / real(V0));
	Vang0.elem(find_nonfinite(Vang0)).zeros();
	Vang0.col(1) = Vang0.col(1) - 180;
	Vang0.col(2) = Vang0.col(2) + 180;
	mat Vabs10;
	Vabs10 << (vo_orig/bkv) << (vo_orig/bkv) << (vo_orig/bkv) << endr;

	mat V0_orig = join_rows(join_rows(join_rows(Vabs10.col(0), Vang0.col(0)), join_rows(Vabs10.col(1), Vang0.col(1))), join_rows(Vabs10.col(2), Vang0.col(2)));

	if (Dl(0, 3) == 14)
	{
		Vpolar = join_cols(V0_orig, Vpolar.rows(0, Lvp - 2));
	}
	else
	{
		mat temp0 = Vpolar.row(Lvp - 1);
		Vpolar = join_cols(temp0, Vpolar.rows(0, Lvp - 2));
	}

	
	cout << "Sending Data to main file.. \n";
	dpf_return123.Vpolar = Vpolar;
	dpf_return123.PQb = PQbus;
	dpf_return123.PQL = PQL;
	dpf_return123.Qset_a = Dl.col(7);
	dpf_return123.Qset_b = Dl.col(9);
	dpf_return123.Qset_c = Dl.col(11);
	return dpf_return123;
}