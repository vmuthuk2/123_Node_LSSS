...
string odss_data;
odss_data = device::COpenDssAdapter::GetData();
//cout << "Dimension of odss_data: " << odss_data.n_rows << "*" << odss_data.n_cols << endl;
cout << "Opendss Data: " << odss_data << endl;

/*************Manipulating Opendss Data*******************/
/*string line;
istringstream f(odss_data);
cout << f << endl;
vector<int> v1, v2;

while (getline(f, line))
{
	cout << line << "with length " << line.length() << endl;
	int value1, value2;
	string fragment1 = line.substr(0,2);
	istringstream(fragment1) >> value1;
	cout << value1 << endl;
	v1.push_back(value1);
	string fragment2 = line.substr(3,2);
	istringstream(fragment2) >> value2;
	cout << value2 << endl;
	v2.push_back(value2);
}

mat final_sol = zeros(v1.size(),1);
for (int j = 0; j < v1.size(); j++)
{
	final_sol(j,0) = v1.at(j);
	final_sol(j,1) = v2.at(j);
}

cout << "Final_Sol: \n" << final_sol << endl;*/


/*stringstream ss(odss_data);
stringstream fs;
string tok;
string::size_type sz;
vector<float> v3, v4;
...
while(getline(ss, tok))
{
	...
	cout << "tok: " << line << endl;
	cout << "after eli: " << line.size() << endl;
	float value3, value4;	
	//float value3 = stof(line, &sz);
	fs << line.substr(0,3);
	fs >> value3;
	fs.clear();
	fs << line.substr(5,2);
	...
	//float value4 = stof(line.substr(sz+1));
	cout << "Value4: " << value4 << endl;
	v3.push_back(value3);
	v4.push_back(value4);
}

mat final_sol = zeros(v3.size(),2);
for (int j = 0; j < v3.size(); j++)
{
	final_sol(j,0) = v3.at(j);
	final_sol(j,1) = v4.at(j);
}
...
while(getline(ss, tok))
{
	...
	else
	{
		...
		fs << tok.substr(1);
		fs >> value2;
		cout << ab << " " << tok.substr(1) << " " << value2 << endl;
	}
	ab++;
...
{
	//cout << "data: " << internal[i] << " of size: " << internal[i].size() << endl;
	//cout << "substr(0,2): " << internal[i].substr(0,2) << endl;
	//cout << "substr(5,2): " << internal[i].substr(5,2) << endl;
	/*if (i == 0)
	{
		float value1 = atof(internal[i].substr(0,3).c_str());
		//fs << tok;
		//fs >> value1;
		//cout << i << " " << value1 << endl;
	}
	...
	{
		float value1 = atof(internal[i].substr(0,2).c_str());
		//fs << tok;
		//fs >> value1;
		cout << i << " " << value1 << endl;
	}
	...
	{
		float value2 = atof(internal[i].substr(5,2).c_str());
		//fs << tok.substr(1);
		//fs >> value2;
		cout << i << " " << value2 << endl;
	}*/
...
{
	string line = tok;	
	cout << "line: " << line << " of length " << line.length() << endl;	
	float val1, val2;
	oss << line.substr(0,2);
	oss >> val1;
	oss.clear();
	...
	pload.push_back(val1);
	pload.push_back(val2); 

}

cout << "Pload Size: " << pload.size() << endl;

mat final_sol = zeros(pload.size(),1);
for (int j = 0; j < pload.size(); j++)
{
	final_sol(j,0) = pload.at(j);
	//final_sol(j,1) = v4.at(j);
}
...
mat Dl = sysinfo.Dl;
//comment here to exclude opendss data 
Dl(1,6) = final_v(0,0);
Dl(1,8) = final_v(1,0);
Dl(1,10) = final_v(2,0);
...
Dl(4,6) = final_v(9,0);
Dl(4,8) = final_v(10,0);
Dl(4,10) = final_v(11,0);

Dl(4,7) = final_v(9,1);
Dl(4,9) = final_v(10,1);
Dl(4,11) = final_v(11,1);

Dl(6,6) = final_v(12,0);
Dl(6,8) = final_v(13,0);
Dl(6,10) = final_v(14,0);

Dl(6,7) = final_v(12,1);
Dl(6,9) = final_v(13,1);
Dl(6,11) = final_v(14,1);

Dl(7,6) = final_v(15,0);
Dl(7,8) = final_v(16,0);
Dl(7,10) = final_v(17,0);

Dl(7,7) = final_v(15,1);
Dl(7,9) = final_v(16,1);
Dl(7,11) = final_v(17,1);

Dl(8,6) = final_v(18,0);
Dl(8,8) = final_v(19,0);
Dl(8,10) = final_v(20,0);

Dl(8,7) = final_v(18,1);
Dl(8,9) = final_v(19,1);
Dl(8,11) = final_v(20,1);
