...
y_re form_Y_abc(arma::mat Dl, arma::cx_mat Z, double bkva, double bkv)
{
	...
	//cout << "brnches: " << brnches << endl;
	//cout << "Lbr" << brnches.n_rows;
	...
	x_wb << 0 << 1 << 3 << 6;
	x_wc << 0 << 1 << 4 << 7;
	cx_mat temp_brnches = y_results.brnches;
	brnches_a(span(0,Lnum_a-1), span(0,3)) = temp_brnches.submat(x_a, x_wa);
	brnches_b(span(0, Lnum_b - 1), span(0, 3)) = temp_brnches.submat(x_b, x_wb);
	brnches_c(span(0, Lnum_c - 1), span(0, 3)) = temp_brnches.submat(x_c, x_wc);
	//cout << "brnches_a\n" << brnches_a << endl;
	...
	lateral_a = lateral_a_temp.rows(0, Lla - 1);
	//cout << "Reached" << endl;
	...
	//main_b = main_b_temp.submat(0, 0, Lmb, Wmb);
	main_b = main_b_temp.rows(0, Lmb-1);
	...
	//lateral_b = lateral_b_temp.submat(0, 0, Llb, Wlb);
	lateral_b = lateral_b_temp.rows(0, Llb-1);
	...
	//main_c = main_c_temp.submat(0, 0, Lmc, Wmc);
	main_c = main_c_temp.rows(0, Lmc-1);
	...
	//lateral_c = lateral_c_temp.submat(0, 0, Llc, Wlc);
	lateral_c = lateral_c_temp.rows(0, Llc-1);
